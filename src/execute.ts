...
import { addDays, generateNextDayPrediction, minMaxInverseScaler, minMaxScaler, processData } from "./helpers.js";
...
export const execute = async (modelTmp: Model, shouldTrain = false) => {
  ...
  const labels = visData.map(val => new Date(val.date).toLocaleString());
  ...
  const nextDayPrediction = generateNextDayPrediction(visResult.originalData, visResult.timePortion);
  // Get the last date from the data set
  const predictDate = addDays((new Date(labels[labels.length - 1])), 1);
  ...
  const trainTensorData = {
    tensorTrainX: tf.tensor1d(trainResult.trainX).reshape([trainResult.size, trainResult.timePortion, 1]),
  ...
  const visTensorData = {
    tensorTrainX: tf.tensor1d(visResult.trainX).reshape([visResult.size, visResult.timePortion, 1]),
    tensorTrainY: tf.tensor1d(visResult.trainY)
  };
  const predictedX = model.predict(visTensorData.tensorTrainX) as tf.Tensor<tf.Rank>;
  // Scale the next day features
  const nextDayPredictionScaled = minMaxScaler(nextDayPrediction, min, max);
  // Transform to tensor data
  const tensorNextDayPrediction = tf.tensor1d(nextDayPredictionScaled.data).reshape([1, visResult.timePortion, 1]);
  // Predict the next day stock price
  const predictedValue = model.predict(tensorNextDayPrediction) as tf.Tensor<tf.Rank>;
  ...
  console.log("Predicted Stock Price of " + symbol + " for date " + predictDate.toLocaleString() + " is: " + inversePredictedValue.data[0].toFixed(3) + "$");
  ...
  await saveChart(symbol, trainYInverse.data as number[], predictedXInverse.data, labels);
}